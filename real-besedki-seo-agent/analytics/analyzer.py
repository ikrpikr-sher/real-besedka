...
def analyze(
    ...
    elif live_og.get("checked") and (live_og.get("product_og_type") or "").lower() != "product":
        findings.append(
            _finding(
                "info",
                "on-page",
                f"og:image есть, og:type={live_og.get('product_og_type') or 'нет'} "
                f"(не product) — это не «нет OG».",
                live_og.get("url") or "",
            )
        )
