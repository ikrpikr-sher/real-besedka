...
class BacklogLiveSkipTests(unittest.TestCase):
    ...
    def test_reopens_blog_h1_on_bare_slug(self) -> None:
        snap = _closed_snapshot()
        snap["site_health"]["checks"]["onpage"]["pages"][-1] = {
            "path": "/blog/category/sovety",
            "h1": ["sovety"],
        }
        sig = live_onpage_signals(snap)
        self.assertFalse(sig["human_blog_h1"])
        tasks = [row["task"] for row in build_backlog(snap)]
        self.assertTrue(any("человекочитаемые H1" in t for t in tasks))
